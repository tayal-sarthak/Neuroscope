...
const ACTION_KEY_PREFIX = 'neuroscope:{analysis-counter}:action:';
...
export function formatAnalysisCount(value) {
    ...
    for (const [divisor, suffix] of units) {
        ...
        const compact = floored.toFixed(decimals).replace(/\.0$/, '');
...
export async function countAnalysisAction(actionId, clientFingerprint) {
    const result = await redisCommand([
        'EVAL',
        COUNT_ACTION_SCRIPT,
        3,
        COUNTER_KEY,
        `${ACTION_KEY_PREFIX}${actionId}`,
